...
(function(webim) {
    ...
    var proto_sendMsg = function(msg, cbOk, cbErr) {
        ...
        msg.elems.forEach(function(item){
            var elem = item;
            var msgContent = null;
            ...
            switch (msgType) {
                ...
                case MSG_ELEMENT_TYPE.IMAGE: //图片
                    var ImageInfoArray = [];
                    elem.content.ImageInfoArray.forEach( function( msgInfo ){
                        ImageInfoArray.push({
                            'Type': msgInfo.type,
                            'Size': msgInfo.size,
                            'Width': msgInfo.width,
        ...
        ConnManager.apiCall(SRV_NAME.OPEN_IM, "getmsg", {
            ...
            function(resp) {
                ...
                if (resp.MsgList && resp.MsgList.length) {
                    resp.MsgList.forEach(function( msg ){
                        tempC2CMsgList.push( msg );
                    })
                }
                if (resp.SyncFlag == 1) {
    ...
    var proto_getC2CHistoryMsgs = function(options, cbOk, cbErr) {
        ...
        ConnManager.apiCall(SRV_NAME.OPEN_IM, "getroammsg", options,
            function(resp) {
                ...
                if (resp.MsgList && resp.MsgList.length) {
                    resp.MsgList.forEach(function( msg ){
                        tempC2CHistoryMsgList.push( msg );
                    })
                }
    ...
    Msg.prototype.toHtml = function() {
        var html = "";        
        this.elems.forEach(function( msg ){
            html += msg.toHtml();
        })
        return html;
    };
